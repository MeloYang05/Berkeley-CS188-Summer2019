    ...
    def elapseTime(self, gameState):
        ...
        for old_pos in self.allPositions:
            ...
            for new_pos in newPosDist.keys():
                if not new_pos in new_beliefs.keys():
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        random.shuffle(self.legalPositions)
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghost_pos in self.particles:
            if not ghost_pos in new_beliefs.keys():
                new_beliefs[ghost_pos] = self.getObservationProb(observation, pac_pos, ghost_pos, jail_pos)
            else:
                new_beliefs[ghost_pos] += self.getObservationProb(observation, pac_pos, ghost_pos, jail_pos)
    ...
    def getBeliefDistribution(self):
        ...
        for i in self.particles:
            if not i in belief_distribution.keys():
                belief_distribution[i] = 1 / len(self.particles)
            else:
                belief_distribution[i] += 1 / len(self.particles)
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        ghosts_pos_list = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        random.shuffle(ghosts_pos_list)
        for i in range(self.numParticles):
            index = i % len(ghosts_pos_list)
            self.particles.append(ghosts_pos_list[index])
    ...
    def observeUpdate(self, observation, gameState):
        ...
        new_particles = list()
        pac_pos = gameState.getPacmanPosition()
        for ghosts_pos in self.particles:
            weight = 1
            for i in range(self.numGhosts):
                jail_pos = self.getJailPosition(i)
                weight *= self.getObservationProb(observation[i], pac_pos, ghosts_pos[i], jail_pos)
            if not ghosts_pos in new_beliefs.keys():
                new_beliefs[ghosts_pos] = weight
            else:
                new_beliefs[ghosts_pos] += weight
        if new_beliefs.total() == 0:
            self.initializeUniformly(gameState)

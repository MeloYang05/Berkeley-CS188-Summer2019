...
class QLearningAgent(ReinforcementAgent):
    ...
    def getQValue(self, state, action):
        ...
        if state not in self.value:
          self.value[state]=util.Counter()
        if action not in self.value[state]:
          self.value[state][action]=0
    ...
    def computeActionFromQValues(self, state):
        ...
        if len(legalActions)==0:
          ...
          for action in legalActions:
            newValues_dict[action]=self.getQValue(state,action)
    ...
    def update(self, state, action, nextState, reward):
        ...
        maxActionValue=self.computeValueFromQValues(nextState)
        self.value[state][action]=(1-self.alpha)*Current_QValue+self.alpha*(reward+self.discount*maxActionValue)
